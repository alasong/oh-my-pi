/**
 * Asset manifest parsing and registry.
 *
 * Reads the declarative `omp.assets.json` at the project root and builds a
 * runtime registry keyed by (type, id). The manifest is the source of truth;
 * the registry is a runtime view derived from it. Adding a new asset dimension
 * is adding a manifest entry, not writing code.
 */
import * as path from "node:path";
import { isEnoent } from "@oh-my-pi/pi-utils";

import type { Asset, AssetManifest, AssetRegistry, AssetType } from "./types";
...
export type { Asset, AssetManifest, AssetRegistry, AssetType, RefreshPolicy } from "./types";
...
/** Build an empty registry (all asset types present, no entries). */
export function createEmptyRegistry(): AssetRegistry {
	...
	for (const type of Object.keys(VALID_TYPES)) {
		registry.set(type as AssetType, new Map());
	}
	return registry;
}

/**
 * Register manifest assets into a registry, keyed by (type, id).
 * Duplicate (type, id) pairs throw — an asset must be uniquely identified.
 */
export function registerAssets(registry: AssetRegistry, assets: Asset[]): void {
	for (const asset of assets) {
		const byType = registry.get(asset.type);
		if (byType === undefined) {
			throw new Error(`Cannot register asset "${asset.id}": unknown type "${asset.type}"`);
		}
		if (byType.has(asset.id)) {
			throw new Error(`Cannot register asset "${asset.id}": duplicate id for type "${asset.type}"`);
		}
		byType.set(asset.id, asset);
	}
}

/**
 * Load the manifest and build a registry in one step. Returns an empty
 * registry when no manifest exists.
 */
export async function loadAssetRegistry(projectRoot: string): Promise<AssetRegistry> {
	const manifest = await loadAssetManifest(projectRoot);
	const registry = createEmptyRegistry();
	registerAssets(registry, manifest.assets);
	return registry;
}

/** Query all assets of a type. */
export function assetsOfType(registry: AssetRegistry, type: AssetType): Asset[] {
	return [...(registry.get(type) ?? new Map()).values()];
}

/** Query a single asset by type and id. */
export function getAsset(registry: AssetRegistry, type: AssetType, id: string): Asset | undefined {
	return registry.get(type)?.get(id);
}
