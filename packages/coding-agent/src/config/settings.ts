...
const SETTING_HOOKS: Partial<Record<SettingPath, SettingHook<any>>> = {
	...
	// pi-tui owns the bash execution component but has no settings access, so the
	// host pushes the resolved values through the shared preference object.
	"bash.lineDisplay": value => {
		if (typeof value === "boolean") setChatTranscriptDisplayPreferences({ lineDisplay: value });
	},
