...
describe("bashToolRenderer", () => {
	...
	it("renders a command's normal negative exit as a warning, keeping the exit code visible", () => {
		// The tool reports a `grep` miss as a non-error result with
		// `expectedNonZeroExit`; without the flag the renderer would paint this
		// as a plain success and drop the exit code.
		const component = bashToolRenderer.renderResult(
			{
				content: [{ type: "text", text: "no matches found\n" }],
				details: { timeoutSeconds: 300, exitCode: 1, expectedNonZeroExit: true },
				isError: false,
			},
			{ expanded: false, isPartial: false },
			uiTheme,
			{ command: "grep -q foo missing.txt" },
		);
		const rendered = component.render(120).join("\n");
		const warningAnsi = uiTheme.fg("warning", "").replace("\x1b[39m", "");
		const errorAnsi = uiTheme.fg("error", "").replace("\x1b[39m", "");
